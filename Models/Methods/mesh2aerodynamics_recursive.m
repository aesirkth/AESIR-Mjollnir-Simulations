function component = mesh2aerodynamics_recursive(component)
...
for child_index = 1:numel(children)
    ...
    component.(child) = mesh2aerodynamics_recursive(component.(child));
    end
...
if isfield(component, 'mesh') 
    ...
    length_scale(dimension) = (abs(max_mesh_height) + abs(min_mesh_height))/2;
    elseif dimension2image_hash(dimension) == "width"
    ...
    length_scale(dimension) = (abs(max_mesh_width) + abs(min_mesh_width))/2;
    end
    ...
    component.aerodynamics.pressure_coefficient         = [0.2;0.2;0.1];
