function [mf_with_crit] = critical_mf_Moody(comp)
    ...
    h1 = py.CoolProp.CoolProp.PropsSI('H', 'P', round(comp.tank.pressure, 2), 'T|liquid', round(comp.tank.liquid.temperature, 2), 'NitrousOxide');   % Enthalpie massic (J/kg)
    s1 = py.CoolProp.CoolProp.PropsSI('S', 'P', round(comp.tank.pressure, 2), 'T|liquid', round(comp.tank.liquid.temperature, 2), 'NitrousOxide');   % Entropy massic (J/K*kg)
    
    %h1 = h1_fun(comp.tank.pressure, comp.tank.liquid.temperature);
    %s1 = s1_fun(comp.tank.pressure, comp.tank.liquid.temperature);
    ...
    %% Equations:
     ...
     x2 = (h2 - hf) ./ h_fg; 
     mf = (h2 <= h1) .* comp.tank.Cd .* Ai .* k ./ (x2 + k .* (1 - x2) .* rho_v ./ rho_l) .* ...
