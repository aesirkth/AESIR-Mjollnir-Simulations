 % Retrieve results.
    ...
    P_cc = state(:, 6) .* empty_tank_mask + comp.P_atm .* (1 - empty_tank_mask);        % The pressure in the combustion chamber at each time step.
    ...
    gam = comp.gamma_combustion_products;               % The heat capacity ratio.
    Mw = comp.molecular_weight_combustion_products;     % The molecular weight of products (kg/mol).
    R = comp.R;                                         % The universal gas constant (J/K/mol).        
    ...
    for i = 1:length(T_tank)
        ...
        if ~ismember(i, empty_tank)
            ...
            P_tank(i) = fnval(comp.Psat_N2O_spline, T_tank(i)) * 10^6;                      % Compute the tank pressure.
            ...
            c_star = interp1q(opts.OF_set, comp.c_star_set, OF(i));                         % Compute the characteristic velocity.
            ...
            F(i) = comp.combustion_efficiency * thrust(mf_throat(i), v_ex(i), P_ext(i), P_ex(i));                       % Compute thrust.
            cp_air(i) = py.CoolProp.CoolProp.PropsSI('C', 'P', P_ext(i), 'T', (T_ext(i) + T_tank_wall(i)) / 2, 'Air');  % Compute constant pressure of air.
            m_fuel(i) = comp.rho_fuel * pi * comp.L_fuel * (comp.D_cc_int^2 / 4 - r_cc(i)^2);                           % Compute fuel mass (density * pi * length * (fuel_r^2 - cc_r^2)).
        else
            % Tank is empty.
            ...
            P_tank(i) = comp.P_atm;             % Take the pressure inside the tank to be equal to the atmospheric pressure.
