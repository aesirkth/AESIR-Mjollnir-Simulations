function [P_cc, mf_ox] = critical_mf_Dyer(comp)
    ...
    P_cc = 1e5:1e5:comp.P_tank;
    mf_SPI = zeros(1, length(P_cc));
    mf_HEM = zeros(1, length(P_cc));
    kappa = ones(1, length(P_cc));
    
    h1 = py.CoolProp.CoolProp.PropsSI('H', 'P', comp.P_tank, 'T|liquid', round(comp.T_tank, 2), 'NitrousOxide');   %Enthalpie massic (J/kg)
    s1 = py.CoolProp.CoolProp.PropsSI('S', 'P', comp.P_tank, 'T|liquid', round(comp.T_tank, 2), 'NitrousOxide');   %Enthalpie massic (J/K.kg)
    
    cp = py.CoolProp.CoolProp.PropsSI('CPMOLAR', 'P', comp.P_tank, 'T|liquid', comp.T_tank, 'NitrousOxide');
    cv = py.CoolProp.CoolProp.PropsSI('CVMOLAR', 'P', comp.P_tank, 'T|liquid', comp.T_tank, 'NitrousOxide');
    gamma = cp / cv;
    
    P_sat = fnval(comp.Psat_N2O_spline, comp.T_tank) * 10^6;
    rho_Ox_1 = fnval(comp.RhoL_T_N2O_spline, comp.T_tank);           %Density of Oxidizer (kg/m^3)
    
    % disp("P_sat : "+P_sat/10^5+" bars")
    for i=1:length(P_cc)
        if P_sat > P_cc(i)
            kappa(i) = sqrt((comp.P_tank - P_cc) / (P_sat - P_cc));
        else
            kappa(i) = 1;
        end
        ...
        mf_SPI(i) = Cd * Ai * sqrt(2 * rho_Ox_1 * (comp.P_tank - P_cc(i)));
    ...
    %%SPI
    critical_mf_SPI = Cd * Ai * sqrt(gamma * rho_Ox_1 * comp.P_tank * (2 / (gamma + 1))^((gamma + 1) / (gamma - 1)));
    %     mf_SPI = min(critical_mf_SPI,mf_SPI);
    % disp("mf_SPI_star : "+critical_mf_SPI)
    
    
    P_star = comp.P_tank * (2 / (gamma + 1))^(gamma / (gamma - 1));
    ...
    %     figure(1)
    %     plot(comp.P_tank-P_cc,mf_ox,comp.P_tank-P_cc,mf_SPI,comp.P_tank-P_cc,mf_HEM)
