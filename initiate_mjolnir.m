...
mjolnir.P_cc_init = 2500000;        % Initial pressure in the combustion chamber (Pa). Needs to be quite high for the model to work.
mjolnir.T_tank_init = 285;          % Initial tank temperature (K).
...
mjolnir.m_ox_init = 24.5;          % Oxidizer mass (kg).
mjolnir.m_fuel_init = 3.1;         % Fuel mass (kg).
mjolnir.rho_ox = 785;              % Oxidizer density (kg/m^3).

mjolnir.m_tot     = 0;              % Total mass (kg) (dependant, computed in simulation).
...
mjolnir.fuel_mass_init = 3.1;      % Initial fuel mass (kg).
mjolnir.rho_fuel = 900;            % Density of fuel (kg/m^3).
mjolnir.r_fuel_init = 5e-2 / 2;    % Fuel port diameter at ignition.
% mjolnir.r_fuel_init = sqrt(mjolnir.D_cc_int^2/4-mjolnir.fuel_mass_init/(mjolnir.rho_fuel*mjolnir.L_fuel*pi));
...
% mjolnir.A_throat_init = pi*(mjolnir.D_throat)^2/4;  % Nozzle throat area (m^2).
Ae_At = 4.75;
...
mjolnir.P_storage_tank_init = fnval(mjolnir.Psat_N2O_spline, mjolnir.T_ext) * 10^6;
