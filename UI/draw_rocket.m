...
%                                                                 [(rocket.rocket.moments.(moment)_of_inertia(3,3).^(0.2))*cos(linspace(0,2*pi,100))*scaler; ...
%                                                                  (rocket.rocket.moments.(moment)_of_inertia(3,3).^(0.2))*sin(linspace(0,2*pi,100))*scaler; ...
%                                                                  zeros(1,100)]*300);
% y_circle =rocket.position +  ...
%                            rocket.attitude*(rocket.center_of_mass + ...
%                                                                 [(rocket.rocket.moments.(moment)_of_inertia(2,2).^(0.2))*cos(linspace(0,2*pi,100))*scaler; ...
%                                                                  zeros(1,100); ...
%                                                                  (rocket.rocket.moments.(moment)_of_inertia(2,2).^(0.2))*sin(linspace(0,2*pi,100))*scaler]*300);
% x_circle = rocket.position +  ...
%                            rocket.attitude*(rocket.center_of_mass + ...
%                                                                 [zeros(1,100); ...
%                                                                  (rocket.rocket.moments.(moment)_of_inertia(1,1).^(0.2))*cos(linspace(0,2*pi,100))*scaler; ...
%                                                                  (rocket.rocket.moments.(moment)_of_inertia(1,1).^(0.2))*sin(linspace(0,2*pi,100))*scaler]*300);
% 
% 
% 
...
%             rocket.attitude(1,:)*scaler, ...
%             rocket.attitude(2,:)*scaler, ...
%             rocket.attitude(3,:)*scaler, ...
%             "Color",ColorMap(end-100,:))

% quiver3(ax, rocket.position(1), ...
%             rocket.position(2), ...
%             rocket.position(3), ...
%             rocket.velocity(1)*scaler, ...
%             rocket.velocity(2)*scaler, ...
%             rocket.velocity(3)*scaler, ...
%             "Color",ColorMap(60,:))
...
%                          rocket.forces.(force).vec(1)*scaler, ...
%                          rocket.forces.(force).vec(2)*scaler, ...
%                          rocket.forces.(force).vec(3)*scaler, ...
%                          "Color", ColorMap(end,:), ...
%                          "LineWidth",1),...
...
cellfun(@(force) quiver3(ax, ...
                         rocket.position(1) +  rocket.attitude(1,:)*rocket.forces.(force).pos - rocket.forces.(force).vec(1)*scaler, ...
                         rocket.position(2) +  rocket.attitude(2,:)*rocket.forces.(force).pos - rocket.forces.(force).vec(2)*scaler, ...
                         rocket.position(3) +  rocket.attitude(3,:)*rocket.forces.(force).pos - rocket.forces.(force).vec(3)*scaler, ...
                         rocket.forces.(force).vec(1)*scaler, ...
                         rocket.forces.(force).vec(2)*scaler, ...
                         rocket.forces.(force).vec(3)*scaler, ...
                         "Color", ColorMap(200,:), ...
...
cellfun(@(force) text(ax, ...
                         rocket.position(1) + rocket.attitude(1,:)*rocket.forces.(force).pos -rocket.forces.(force).vec(1)*scaler*0.2 + 0.3, ...
                         rocket.position(2) + rocket.attitude(2,:)*rocket.forces.(force).pos -rocket.forces.(force).vec(2)*scaler*0.2 + 0.3, ...
                         rocket.position(3) + rocket.attitude(3,:)*rocket.forces.(force).pos -rocket.forces.(force).vec(3)*scaler*0.2 + 0.3, ...
...
%                          rocket.moments.(moment).vec(1)*scaler*300, ...
%                          rocket.moments.(moment).vec(2)*scaler*300, ...
%                          rocket.moments.(moment).vec(3)*scaler*300, ...
%                          "Color", ColorMap(60,:)),...
%          values(rocket.rocket.moments.(moment)s , "cell"))

cellfun(@(moment) quiver3(ax, ...
                         rocket.position(1) + rocket.attitude(1,:)*rocket.moments.(moment).pos - rocket.moments.(moment).vec(1)*scaler*10, ...
                         rocket.position(2) + rocket.attitude(2,:)*rocket.moments.(moment).pos - rocket.moments.(moment).vec(2)*scaler*10, ...
                         rocket.position(3) + rocket.attitude(3,:)*rocket.moments.(moment).pos - rocket.moments.(moment).vec(3)*scaler*10, ...
                         rocket.moments.(moment).vec(1)*scaler*10, ...
                         rocket.moments.(moment).vec(2)*scaler*10, ...
                         rocket.moments.(moment).vec(3)*scaler*10, ...
                         "Color", ColorMap(150,:), ...
                         "LineWidth", 2),...
         fieldnames(rocket.moments))

cellfun(@(moment) text(ax, ...
                         rocket.position(1) + rocket.attitude(1,:)*rocket.moments.(moment).pos - rocket.moments.(moment).vec(1)*scaler*4 + 0.3, ...
                         rocket.position(2) + rocket.attitude(2,:)*rocket.moments.(moment).pos - rocket.moments.(moment).vec(2)*scaler*4 + 0.3, ...
                         rocket.position(3) + rocket.attitude(3,:)*rocket.moments.(moment).pos - rocket.moments.(moment).vec(3)*scaler*4 + 0.3, ...
